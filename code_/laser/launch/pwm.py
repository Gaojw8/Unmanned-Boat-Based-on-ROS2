...
class MotorController(Node):
    def __init__(self, name):
        ...
        self.motor1_pwm_pin = 12  # 硬件PWM GPIO12 (BCM编码)
        self.motor2_pwm_pin = 13  # 硬件PWM GPIO13 (BCM编码)
    ...
    def cmd_vel_callback(self, msg):
        ...
        if linear == 0.0:
            ...
            if angular == 0.0:
                # 前进
                self.set_pwm(1600, 1400)
            if angular > 0.0:
                self.set_pwm(1200, 1250)
            if angular < 0.0:
                self.set_pwm(1750, 1800)
        elif linear == 0.5:
            self.set_pwm(1650, 1350)
        elif linear == -0.2:
            if angular == 0.0:
                # 前进
                self.set_pwm(1400, 1600)
            if angular > 0.0:
                self.set_pwm(1250, 1200)
            if angular < 0.0:
                self.set_pwm(1800, 1750)
        elif linear == -0.5:
            self.set_pwm(1350, 1650)
